...
- **`conventional-pre-commit` must stay declared BEFORE `giticket`** in `.pre-commit-config.yaml`. Both run at the `commit-msg` stage and pre-commit runs them in declaration order. giticket rewrites the subject to `[OND211-2418] feat: …`, which is no longer valid Conventional Commits, so with giticket first **every** commit on a ticket branch failed the Conventional Commit hook and could only land with `--no-verify` (fixed 2026-07-29, OND211-2418). Validate first, decorate second.
- **The hook chain needs the repo-root `node_modules`, not just `src/node_modules`.** `.husky/pre-commit` runs `make eslint` / `make prettier`, whose `./node_modules/.bin/…` binaries live at the **repo root** (the codegen only needs `src/node_modules`). Install with `make install_dependencies` — plain `npm ci` fails ERESOLVE because `@angular/platform-browser@^20.3.25` wants peer `@angular/common@20.3.25` while `@angular/common` is pinned to `20.1.7`; the repo's target uses `--legacy-peer-deps` on purpose. Regenerate stubs with: point `src/ondewo-nlu-api` at the api commit → `cd src && npm run build` (Docker `generate` + compile to `../npm/`) → `make test-in-ondewo-aim-copy-only`.
- **On an nvm box, git hooks cannot see `node`.** If `node`/`npm` are nvm shell functions (oh-my-zsh lazy-loading) they exist only in interactive zsh, and git runs hooks under a non-interactive `/bin/sh` → every `#!/usr/bin/env node` shebang dies with `env: 'node': No such file or directory`. Fix it **per machine, not in the repo**: husky v9 sources `~/.config/husky/init.sh` before each hook, and `make run_precommit_hooks` calls `.husky/pre-commit` directly (bypassing that wrapper), so it additionally needs a real `node` on `PATH` — e.g. symlinks in `~/.local/bin`.
- **`README.md` and `index.d.ts` are prettier-ignored because they are generated.** Root `README.md` is a copy of `src/README.md` (`src/package.json` `postbuild`) and `index.d.ts` is codegen output. Beyond the churn, prettier rewrites `[comment]: <> (START OF GITHUB README)` into `[comment]: <> 'START OF GITHUB README'` — the same marker hazard as MD053 below.
...
```text
Release ONDEWO NLU Angular Client ${ONDEWO_NLU_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO NLU Angular Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO NLU Angular Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/nlu-client-angular`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/nlu-client-angular versions --json
```
